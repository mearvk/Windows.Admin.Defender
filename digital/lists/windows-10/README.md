...
The Windows 10 protected-system baseline incorporates the folder/file series defined in `sacred-files.json`. These locations should be treated as protected from wholesale copying, replacement, deletion, or uncontrolled alteration except through appropriate Windows administration, servicing, deployment, recovery, or application-management procedures.
...
The baseline is derived from the Windows 10 `sacred-files.json` collection and should be extended when additional Windows-managed locations are identified.
